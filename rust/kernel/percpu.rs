...
impl<T> PerCpuPtr<T> {
    ...
    /// Get a [`*mut MaybeUninit<T>`](MaybeUninit) to the per-CPU variable represented by `&self`
    /// on the current CPU. Note that if CPU preemption is not disabled before calling this
    ...
    pub fn get_ptr(&self) -> *mut MaybeUninit<T> {
        // SAFETY: `this_cpu_ptr` has no preconditions
        unsafe { bindings::this_cpu_ptr(self.0 as *mut c_void) as *mut MaybeUninit<T> }
    }

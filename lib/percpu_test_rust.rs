...
macro_rules! make_optimization_test {
    ($ty:ty) => {
        let mut test: PerCpu<$ty> = PerCpu::new(GFP_KERNEL).unwrap();
        {
            let _ = CpuGuard::new();
...
impl kernel::Module for PerCpuTestModule {
    fn init(_module: &'static ThisModule) -> Result<Self, Error> {
        ...
        let mut pcpu: PerCpu<i64> = unsafe { unsafe_get_per_cpu!(PERCPU) };
        ...
        let mut upcpu: PerCpu<u64> = unsafe { unsafe_get_per_cpu!(UPERCPU) };
        ...
        let mut test: PerCpu<u64> = PerCpu::new(GFP_KERNEL).unwrap();
